...
export async function exportCSV(orgId: string) {
  ...
  if (memberError || !member || !canExportTransactions(member.role)) {
    return {
      error:
        "You do not have permission to export transactions for this organization.",
      code: "no_permission",
    };
  }
  ...
  if (!data || data.length === 0) {
    return {
      error: "Your organization does not have any transactions to export.",
      code: "no_data",
    };
  }
